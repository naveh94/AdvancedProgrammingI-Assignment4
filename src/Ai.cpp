...
int Ai::calculateBestMove(vector<Point> available_moves)
{
    ...
    game_copy.playTurn(available_moves.at(0).getX(),available_moves.at(0).getY());
    ...
    // moves.
    worst_score = current_score + game_copy.availableMoves();
    for (int i = 1; i < available_moves.size(); i++)
    {
        game_copy = new Game(game);
        game_copy.playTurn(available_moves.at(i).getX(),available_moves.at(i).getY());
        current_score = game_copy.getCurrentScore(BLACK_PLAYER);
        ...
        if (temp =  current_score + game_copy.availableMoves() < worst_score)
        {
            worst_score = temp;
